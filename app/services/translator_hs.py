...
gemini_model_1 = genai.GenerativeModel(
    "gemini-2.0-flash-lite")  # type: ignore # gemini-1.5-flash
...
async def _classify_gemini(strings_batch, batch_num, total_batches, classification_model):
    """
    Classify strings into 'business' or 'ordinary'.
    """
...
async def _translate_batch(indexed_strings, target_lang, brand_tone, batch_num, total_batches, type, translation_progress=None):
        ...
        if translations:
            ...
            if expected == got:
                if translation_progress is not None:
                    ...
                    # print(
                    #     f"[TRANSLATION PROGRESS: valid] {translation_progress['valid']} valid, {translation_progress['partial']} partial, total {translation_progress['valid']+translation_progress['partial']}/{translation_progress['total']} (batch {batch_num} via {current_model})")
            ...
            # --- FIX: force align translations ---
                ...
                # print(
                #     f"Expected {expected}, got {got} -> Padding {expected-got} from original batch")
                translations.extend(strings[got:])
            else:  # got > expected
                # Trim extras
                translations = translations[:expected]
                # print(
                #     f"Expected {expected}, got {got} -> Truncating {got-expected} from translation batch")
            ...
            if translation_progress is not None:
                ...
                # print(f"[TRANSLATION PROGRESS: partial] {translation_progress['valid']} valid, {translation_progress['partial']} partial, total {translation_progress['valid']+translation_progress['partial']}/{translation_progress['total']} (batch {batch_num} via {current_model})")
            return [(i, t) for (i, _), t in zip(indexed_strings, translations)]
...
async def fast_translate_json(target_data, target_lang, brand_tone):
    ...
    strings_to_classify = [(i, s) for i, s in enumerate(strings_to_translate)]
    ...
    # ---- TRANSLATE ----
    batches = [strings_to_classify[i:i+BATCH_SIZE]
               for i in range(0, len(strings_to_classify), BATCH_SIZE)]
    total_batches = len(batches)

    start = datetime.now()

    classification_progress = {"valid": 0,
                               "partial": 0, "total": total_batches}
    ...
    classified = [(i, s, l)
                  for i, (s, l) in enumerate(zip(strings_to_translate, final_results))]
    ...
    for idx, batch in enumerate(business_batches):
        translation_tasks.append(_translate_batch(batch, target_lang, brand_tone, idx+1,
                                                  len(business_batches), type="business", translation_progress=translation_progress))

    for idx, batch in enumerate(ordinary_batches):
        translation_tasks.append(_translate_batch(batch, target_lang, brand_tone, idx+1,
                                                  len(ordinary_batches), type="ordinary", translation_progress=translation_progress))
    ...
    final_translation_pairs = [pair
                               for batch in all_translation_results if batch is not None
                               for pair in batch]
