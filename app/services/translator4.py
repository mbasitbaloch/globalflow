...
async def _translate_batch(indexed_strings, target_lang, brand_tone, batch_num, total_batches, type, progress=None):
    ...
    async with semaphore:
        ...
        try:
            ...
            if not translations or len(translations) != len(strings):
                raise ValueError(
                    f"Length mismatch: got {len(translations) if translations else 0}, expected {len(strings)}"
                )
            ...
            for alt in model_cycle:
                ...
                try:
                    ...
                    if not translations or len(translations) != len(strings):
                        raise ValueError(
                            f"Fallback {alt} length mismatch: got {len(translations) if translations else 0}, expected {len(strings)}"
                        )
                    break
                except Exception as e2:
                    print(f"⚠ Fallback {alt} also failed: {e2}")
