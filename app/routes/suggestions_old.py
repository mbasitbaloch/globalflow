...
print("Entered in suggestions_old.py")
...
async def suggestions_generate(req: GenerateRequest, db: Session = Depends(get_db)):
   ...
   # 1 Validate all required fields exist and are non-empty
    ...
    try:
        ...
        # db.commit()  # commit ONCE only!

        # store suggestions in DB (for audit & indexing)
        for seg in out["suggestions"]:
            s_model = SuggestionSegment(
                tenant_id=req.tenant_id,
                translation_id=req.translation_id,
                doc_type=req.doc_type,
                domain=req.domain,
            ...
            segment_id = s_model.id

            # ---- (B) Save SuggestionOptions for this segment ----
            option_items = []
            for opt in seg["suggestions"]:
                opt_model = SuggestionOption(
                    segment_id=segment_id,
                    suggestion_text=opt.get("text"),
                    score=opt.get("score")
                )
                db.add(opt_model)
                db.flush()     # get ID
                ...
                option_items.append({
                    "id": opt_model.id,
                    "text": opt_model.suggestion_text,
                    "score": opt_model.score
                })
